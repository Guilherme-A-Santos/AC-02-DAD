...
def nome_do_pokemon(numero):
    possiveis_numeros = range(1, 808)
    if numero not in possiveis_numeros:
        raise PokemonNaoExisteException
    req = requests.get(f"http://localhost:8000/api/v2/pokemon/{numero}")
    dic_poke = req.json()
    return dic_poke["name"]
...
def evolucoes_proximas(nome):
    ...
    req = requests.get(f"http://localhost:8000/api/v2/pokemon-species/{nome}/")
    dic_poke = req.json()
    dic_chain = dic_poke['evolution_chain']['url']
    req1 = requests.get(dic_chain)
    dic_evo = req1.json()
    d1 = dic_evo['chain']
    lista_evolucoes = []
    if d1['species']['name'] == nome:
        d2 = d1['evolves_to']
        if d2 != []:
            d3 = d2[0]
            lista_evolucoes.append(d3['species']['name'])
    elif d1['species']['name'] != nome:
        d2 = d1['evolves_to']
        if d2 != []:
            d3 = d2[0]
            if d3['species']['name'] == nome:
                d4 = d3['evolves_to']
                d5 = d4[0]
                lista_evolucoes.append(d5['species']['name'])
...
if __name__ == "__main__":
    print(evolucoes_proximas("ninjask"))
